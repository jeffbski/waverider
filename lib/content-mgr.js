...
/**
 * take simply fn(cb) and once it has been called successfully
 * reuse the same result on successive calls
 */
function memoize(fn) {
  /*jshint validthis:true */
  var memo;
  ...
  return function (cb) {
    if (!called) {
      ...
      fn(function (err, result) {
        if (err) {
          listeners.forEach(function (lcb) { lcb(err); });
          return cb(err);
        }
        memo = result;
        cb(null, memo);
        listeners.forEach(function (lcb) { lcb(null, memo); });
      });
    } else { // was called
      if (typeof memo !== 'undefined') return cb(null, memo); // have value, use immediately
      listeners.push(cb);
    }
  };
}
...
function setStream(key, inStream, type, meta, cb) {
  ...
  var digester = digest();
  var length = 0;
  // since need to apply these input handlers asap, using memoize
  // to get value once, probably need to simplify this using readstream
  var memoGetNextContentId = memoize(getNextContentId);
  inStream.on('error', function (err) { cb(err); });
  inStream.on('data', function (data) {
    memoGetNextContentId(function (err, contentId) {
      if (err) return cb(err);
